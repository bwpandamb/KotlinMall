...
class CartFragment : BaseMvpFragment<CartListPresenter>(), CartListView {
    ...
    private fun initView() {
        ...
        mDeleteBtn.onClick {
            ...
            if (cartIdList.size == 0) {
                ...
                mPersenter.deleteCartList(cartIdList)
            }
        }
        ...
        mSettleAccountsBtn.onClick {
            ...
            if (cartGoodsList.size == 0) {
                ...
                mPersenter.submitCart(cartGoodsList, mTotalPrice)
            }
        }
    }
    ...
    private fun initObserve() {
        Bus.observe<CartAllCheckedEvent>().subscribe { t: CartAllCheckedEvent ->
            //这里不用run 也是可以的，不过用不用不影响，可以查查看Kotlin系列之let、with、run、apply、also函数的使用
            run {
    ...
    private fun updateTotalPrice() {
        ...
        //关于计算总价。这几个集合的操作符可以好好看看 https://www.2cto.com/kf/201611/567100.html
        mTotalPrice = mAdapter.dataList
                .filter { it.isSelected } //filter 先过滤已选中的元素
                .map { it.goodsCount * it.goodsPrice } //map 将集合中的元素通过某个 方法转换 后的结果存到一个集合中
                .sum() //sum 再加合
