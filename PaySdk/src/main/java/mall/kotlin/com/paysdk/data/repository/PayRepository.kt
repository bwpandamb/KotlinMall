...
class PayRepository @Inject constructor() {
    ...
    fun getPaySign(orderId: Int, totalPrice: Long): Observable<BaseResp<String>> {
        return RetrofitFactory.instance.creat(PayApi::class.java).getPaySign(GetPaySignReq(orderId, totalPrice))
    }
    ...
    fun payOrder(orderId: Int): Observable<BaseResp<String>> {
        return RetrofitFactory.instance.creat(PayApi::class.java).payOrder(PayOrderReq(orderId))
    }
