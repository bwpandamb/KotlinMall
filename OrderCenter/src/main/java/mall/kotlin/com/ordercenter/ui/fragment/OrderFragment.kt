...
class OrderFragment : BaseMvpFragment<OrderListPresenter>(), OrderListView {
    ...
    private fun initObserve() {
        Bus.observe<ChangeTabEvent>()
                .subscribe { t: ChangeTabEvent ->
                    mPersenter.getOrderList(t.position)
                }
                .registerInBus(this)
            ...
            override fun onOptClick(optType: Int, order: Order) {
                when (optType) {
                    ...
                    OrderConstant.OPT_ORDER_CONFIRM -> {
                        mPersenter.confirmOrder(order.id)

                    }
                    OrderConstant.OPT_ORDER_CANCEL -> {
                        //mPersenter.cancelOrder(order.id)
    ...
    private fun loadData() {
        ...
        mPersenter.getOrderList(arguments.getInt(OrderConstant.KEY_ORDER_STATUS, -1))
    }
